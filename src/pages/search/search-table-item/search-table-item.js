...
const SearchTableItem = (props) => {
    ...
    useEffect(()=>{
        setAdminNote(props.data.admin_note)
        const fd = new FormData
        fd.append('user_id',props.data.company_id)
    ...
    const cancelTransfer = () => {
        ...
        axios.post(`${process.env.REACT_APP_BASE_API}update-status`,fd)
            .then(res => {
                if(res.data === 'updated'){
    ...
    const updateStatus = (status)=>{
        ...
        fd.append('company_id',e.company_id)
        fd.append('order_amount',e.order_amount)
        axios.post(`${process.env.REACT_APP_BASE_API}update-status`,fd)
            .then(res=>{
                if(res.data === 'updated'){
                    alert('Status updated')
    ...
    const handleNote = ()=>{
        const fd = new FormData
        fd.append('id',e.id)
        fd.append('note',adminNote)
        axios.post(`${process.env.REACT_APP_BASE_API}update-note`,fd)
            .then(res=>{
                if(res.data === 'updated'){
                    alert('Note Saved')
                }
    ...
    return (
        <tr>
            ...
            {localStorage.getItem('user') === 'admin' ?
                ...
                : ''}
            {e.status !== 'completed' && localStorage.getItem('user') !== 'admin' ? <td className='text-center'>
                <p className='cancelButton m-auto' onClick={cancelTransfer}>Cancel</p>
            </td> :''}
            {localStorage.getItem('user') === 'admin' ?<td>
                <div className="d-flex">
                    <input style={{textIndent:'15px'}} placeholder='Note' type="text" value={adminNote} onChange={(e)=>{setAdminNote(e.target.value)}}/>
